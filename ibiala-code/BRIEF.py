...
def makeTestPattern(patch_width=9, nbits=256):
    ...
    '''
    compareX = np.random.randint(0, high=patch_width ** 2, size=nbits)
    compareY = np.random.randint(0, high=patch_width ** 2, size=nbits)
...
def computeBrief(im, gaussian_pyramid, locsDoG, k, levels,
    ...
    locs = []
    desc = np.empty((0, nbits))

    for i in range(locsDoG.shape[0]):
        keypoint = locsDoG[i, :]
        y, x, level = keypoint
        # Check that patch fully fits based on keypoint coordinates
        if (x - patch_size//2 >= 0) and (y - patch_size//2 >= 0) and (x + patch_size//2 < im.shape[0]) and (y + patch_size//2 < im.shape[1]):
            locs.append(keypoint)

            # Create the patch
            patch = np.empty((patch_size, patch_size))
            for i in range(patch_size):
                patch[i, :] = im[x - patch_size//2 + i, y - patch_size//2:y + patch_size//2 + 1]
            patch.resize(patch_size ** 2)

            # Compare pixel values
            descriptor = np.zeros((1, nbits))
